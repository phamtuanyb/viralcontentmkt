...
import { bannersApi, type HomepageBanner } from "@/api/banners.api";
...
import { Image, Plus, Edit, Trash2, Loader2, Eye, EyeOff } from "lucide-react";
...
const AdminBannersPage = () => {
  ...
  const fetchBanners = async () => {
    setIsLoading(true);
    const { data } = await bannersApi.getAllHomepageBanners();
  ...
  const handleOpenDialog = (banner?: HomepageBanner) => {
    if (banner) {
      ...
      setFormData({ title: "", image_url: "", link_url: "", is_active: true, sort_order: 0 });
    }
  ...
  const handleSubmit = async () => {
    ...
    if (editingBanner) {
      const { error } = await bannersApi.updateHomepageBanner(editingBanner.id, formData);
      if (error) {
      ...
      const { error } = await bannersApi.createHomepageBanner(formData);
      if (error) {
        toast({ title: "Lỗi", description: error.message, variant: "destructive" });
      } else {
  ...
  const handleDelete = async (id: string) => {
    ...
    const { error } = await bannersApi.deleteHomepageBanner(id);
    if (error) {
      toast({ title: "Lỗi", description: error.message, variant: "destructive" });
    } else {
      ...
      >
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Banner trang chủ</h1>
          </div>
