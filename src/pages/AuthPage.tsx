...
const AuthPage = () => {
  ...
  const from = (location.state as { from?: { pathname: string } })?.from?.pathname || ROUTES.CONTENT_LIBRARY;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        const { error } = await authApi.signIn(formData.email, formData.password);
        if (error) {
        ...
        if (error) {
          if (error.message.includes("already registered")) {
            toast({ title: "Lỗi", description: "Email đã được đăng ký", variant: "destructive" });
          } else {
            toast({ title: "Lỗi", description: error.message, variant: "destructive" });
          }
        } else {
