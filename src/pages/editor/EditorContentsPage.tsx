...
const EditorContentsPage = () => {
  ...
  const fetchData = async () => {
    ...
    const [contentsRes, topicsRes] = await Promise.all([
      contentApi.getByCreator(user.id),
      topicsApi.getAll(),
    ]);
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground">
            Quản lý và tạo nội dung của bạn
          </p>
        </div>
        <Button 
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              {contents.length === 0 && "Bắt đầu tạo bài viết đầu tiên của bạn"}
            </p>
            {contents.length === 0 && (
              <Button onClick={() => navigate(ROUTES.MY_CONTENT_NEW)} className="gap-2">
                <Plus className="h-4 w-4" />
                Tạo bài viết mới
              </Button>
            )}
                    ...
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {content.is_published && (
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => window.open(`/content/${content.short_id || content.id}`, "_blank")}
                          title="Xem"
                        >
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => navigate(`/my-contents/${content.id}/edit`)}
                        title="Chỉnh sửa"
                      >
...
export default EditorContentsPage;
