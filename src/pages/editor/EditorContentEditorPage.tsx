...
const EditorContentEditorPage = () => {
    ...
    const fetchData = async () => {
      ...
      if (id) {
        ...
        if (contentRes.data) {
          ...
          if (contentRes.data.created_by !== user?.id) {
            toast({ title: "Lỗi", description: "Bạn không có quyền chỉnh sửa bài viết này", variant: "destructive" });
            navigate(ROUTES.MY_CONTENTS);
            return;
          }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Thành công", description: isEditMode ? "Đã cập nhật bài viết" : "Đã tạo bài viết mới" });
      setHasChanges(false);
      navigate(ROUTES.MY_CONTENTS);
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <header className="sticky top-16 z-40 glass-strong border-b border-border">
        <div className="flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-4">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => navigate(ROUTES.MY_CONTENTS)}
              className="gap-2"
            >
      ...
      <div className="flex">
            ...
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "edit" | "preview")}>
              ...
              <TabsContent value="edit" className="mt-4">
                ...
                <div className="flex items-center gap-1 p-2 rounded-t-lg bg-muted/50 border border-b-0 border-border flex-wrap">
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleToolbarAction("bold")} title="Đậm">
                    <Bold className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleToolbarAction("italic")} title="Nghiêng">
                    <Italic className="h-4 w-4" />
                  </Button>
                  <div className="h-6 w-px bg-border mx-1" />
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleToolbarAction("h1")} title="Tiêu đề 1">
                    <Heading1 className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleToolbarAction("h2")} title="Tiêu đề 2">
                    <Heading2 className="h-4 w-4" />
                  </Button>
                  <div className="h-6 w-px bg-border mx-1" />
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleToolbarAction("list")} title="Danh sách">
                    <List className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleToolbarAction("ordered-list")} title="Danh sách có số">
                    <ListOrdered className="h-4 w-4" />
                  </Button>
                  <div className="h-6 w-px bg-border mx-1" />
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleToolbarAction("quote")} title="Trích dẫn">
                    <Quote className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleToolbarAction("code")} title="Code">
                    <Code className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => handleToolbarAction("link")} title="Liên kết">
                ...
                <Textarea
                  ...
                  className="min-h-[400px] rounded-t-none border-t-0 resize-none font-mono text-base leading-relaxed"
                />
              </TabsContent>
        ...
        {/* Sidebar */}
        <aside className="w-80 border-l border-border p-6 space-y-6 hidden lg:block">
          <div>
            <Label className="text-sm font-medium mb-2 block">Chủ đề</Label>
            <Select value={formData.topic_id} onValueChange={(v) => updateFormData({ topic_id: v })}>
              <SelectTrigger>
...
export default EditorContentEditorPage;
